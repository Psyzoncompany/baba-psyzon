...
test('versao global possui um unico valor e abre o historico', () => {
  ...
  assert.match(component, /const SITE_VERSION = '6\.0\.44'/);
...
test('rodape global e carregado nas paginas comuns, Baba e mesa tatica', () => {
  ...
  assert.match(preloader, /site-version\.js\?v=6\.0\.44/);
  assert.match(read('baba.html'), /site-version\.js\?v=6\.0\.44/);
  assert.match(read('mesa-tatica.html'), /site-version\.js\?v=6\.0\.44/);
...
test('pagina de versoes apresenta a atualizacao atual e seu conteudo', () => {
  ...
  assert.match(versions, /id="version-6-0-44"/);
  assert.match(versions, /Versão 6\.0\.44/);
  assert.match(versions, /Cada conta Google agora possui seu próprio espaço de dados/);
  assert.match(versions, /Novo botão global de versão no rodapé/);
});
