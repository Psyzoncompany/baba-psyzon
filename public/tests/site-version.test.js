...
test('versao global possui um unico valor e abre o historico', () => {
  ...
  assert.match(component, /const SITE_VERSION = '6\.0\.46'/);
...
test('rodape global e carregado nas paginas comuns, Baba e mesa tatica', () => {
  ...
  assert.match(preloader, /site-version\.js\?v=6\.0\.46/);
  assert.match(read('baba.html'), /site-version\.js\?v=6\.0\.46/);
  assert.match(read('mesa-tatica.html'), /site-version\.js\?v=6\.0\.46/);
...
test('pagina de versoes apresenta a atualizacao atual e seu conteudo', () => {
  ...
  assert.match(versions, /id="version-6-0-46"/);
  assert.match(versions, /Versão 6\.0\.46/);
  assert.match(versions, /quatro dígitos/);
  assert.match(versions, /exportação dos times em TXT/);
});
